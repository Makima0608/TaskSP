...
class ABLECORESP_API FAbleAbilityDebug
{
public:
	...
	static void DrawConeSliceQuery(const UWorld* World, const FTransform& QueryTransform, float FOV, float Length, float Height, float Duration = 0.5f, FLinearColor DebugColor = FLinearColor::Red);

	/* Draws a 3D Cone Query using a base radius */
	static void DrawConeQueryWithRadius(const UWorld* World, const FTransform& QueryTransform, float Radius, float Length);
