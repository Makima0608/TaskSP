...
#include "AnimNode_SPAbilityAnimPlayer.generated.h"

class UAnimSequence;

USTRUCT()
struct ABLECORESP_API FSPAbilityAnimEntry
{
	GENERATED_BODY()
	FSPAbilityAnimEntry();
	FSPAbilityAnimEntry(const UAnimSequence* InSequence, const FAlphaBlend& InBlend, const FAlphaBlend& OutBlend, float InPlayRate);
public:
...
USTRUCT()
struct ABLECORESP_API FAnimNode_SPAbilityAnimPlayer : public FAnimNode_AssetPlayerBase
{
	GENERATED_BODY()

	FAnimNode_SPAbilityAnimPlayer();
...
private:
	...
	UPROPERTY()
	TArray<FSPAbilityAnimEntry> m_AnimationQueue;
