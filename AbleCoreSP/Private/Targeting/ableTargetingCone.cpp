...
#include "Targeting/ableTargetingCone.h"
...
UAbleTargetingCone::UAbleTargetingCone(const FObjectInitializer& ObjectInitializer)
...
UAbleTargetingCone::~UAbleTargetingCone()
{
...
void UAbleTargetingCone::FindTargets(UAbleAbilityContext& Context) const
{
...
void UAbleTargetingCone::BindDynamicDelegates(class UAbleAbility* Ability)
{
...
float UAbleTargetingCone::CalculateRange() const
{
	if (Is2DQuery())
	{
...
void UAbleTargetingCone::ProcessResults(UAbleAbilityContext& Context, const TArray<struct FOverlapResult>& Results, float _FOV, float _Height, float _Length) const
{
	// Our output
...
#if WITH_EDITOR
void UAbleTargetingCone::OnAbilityEditorTick(const UAbleAbilityContext& Context, float DeltaTime) const
{
	// Draw a Preview.
